...
for ($i=1; $i < 4; $i++) 
{
    if($impresion['x_hoja'] == $i)
    {
        $opciones_x_hoja .= '<option value="'.$i.'" selected>'.$i.'</option>';        
    }else
    {
        $opciones_x_hoja .= '<option value="'.$i.'">'.$i.'</option>';    
    }     	
}

$html .= '<div class="form-group">';					
$html .= setLabel($this->lang->line('x_hoja'));
$html .= setLabel('<select '.completar_tag('x_hoja', $impresion).'>'.$opciones_x_hoja.'</select>', 2, 'input');
$html .= '</div>';			
