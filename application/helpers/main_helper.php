...
- table_upd				Button para eliminar varios registros
...
function setTableContent($registros){
	$content = '<tr>';
...
function setCheckbox($check, $registro, $checked = NULL){
	...
	$return .= '<div class="col-sm-2 text-center">';
	...
	$return .= '</div>';		
	$return .= '<div class="col-sm-8">'.lang('info_'.$check).'</div>';				
	$return .= '</div>';	
	$return .= '</div>';		
...
function setBoxSolid($header, $body, $size = NULL){
	...
	if(is_array($header)){
		$box .=
		'<div class="box box-solid box-'.$header['class'].'">
