 ...
 --------------------------------------------------------------------------------*/	
	...
	public function ajax($id_afiliado = NULL)
	{
		if($this->_session_data['id_perfil'] == 2){
		    ...
		    if($this->_config['entes_delete'] == 1){
               $registros    = $this->m_entes->getRegistros('all'); 
            }else{
		...
		if($registros){
			...
			$icon_estado_ok  = "'fa fa-check'";
            $icon_estado_no  = "'fa fa-trash'";
            $btn_class_ok    = "'label label-success'";
            $btn_class_no    = "'label label-danger'";
			...
			foreach ($registros as $row) {
                ...
                if($this->_config['entes_delete'] == 1){
                    if($row->eliminado == 0){
                        $estado = '<span class='.$btn_class_ok.'><i class='.$icon_estado_ok.'></i></span>';
                    } else {
                        $estado = '<span class='.$btn_class_no.'><i class='.$icon_estado_no.'></i></span>';
                    }
 ...
 --------------------------------------------------------------------------------*/	
	...
	function notificacionSiris($id, $accion, $method = NULL)
	{
		...
		if($registro){
            ...
            try 
            {
                $result		= @file_get_contents($this->_config['url_conexion_siris'], false, $context);
            }catch (Exception $e) 
            {
                $return['flag']     = 'error';
                $return['result']   = 'No hay conexión con '.$e->getMessage().'<br>';
                log_message('error', 'Error en ENTES '.$e->getMessage());
            }
            
			if($result === FALSE)
			{
				$return['flag']		= 'error';
				$return['result']	= 'No hay conexión con '.$this->_config['url_conexion_siris'].'<br>';
                log_message('error', 'Error en ENTES '.$return['result']);
                log_message('error', 'result=  '.print_r($result, TRUE));
			}else if($result != 'Ok')
			{
				$return['flag']		= 'error';
				//$return['result']	= $result.' al '.$accion.' el ente id:'.$id.' codigo:'.$codigo.'<br>';
				$return['result']   = '<a href="'.base_url().'index.php/entes/abm/'.$id.'">Error de conexión con Siris, el ente '.$id.' se ha dado de baja, es necesario sincronizarlo</a><br>';
                log_message('error', 'Error en ENTES '.$return['result']);
                log_message('error', 'result=  '.print_r($result, TRUE));
			}else
			{
			...
			} else 
			{
				if($return['flag'] == 'error')
				{
					...
					$this->alerta_banco($return['result']);
				}else
				{
